...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # causal mask. register_buffer 是 PyTorch 中 nn.Module 的一个重要方法，主要用于注册不需要梯度计算的张量(tensor)
    ...
    def forward(self, x):
        ...
        # att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1))) # 对hs维度进行归一化
        ...
        #  在进行维度变换后，有时张量在内存中的存储可能不连续 
        # 某些操作（如 view）要求张量必须是连续的
        y = self.c_proj(y)
        return y
...
class GPT(nn.Module):
    ...
    def _init_weight(self, module):
        if isinstance(module, nn.Linear):
            ...
            if hasattr(module, "NANOGPT_SCALE_INIT"):
                std *= (2 * self.config.n_layer) ** -0.5 # 每个transformer层中有两个residual
    ...
    def forward(self, idx, targets=None):
        ...
        # forward the transforemr blocks
        for block in self.transformer.h:
            x = block(x)

        # forward the final layernorm and lm_head
    ...
    # @classmethod 是 Python 的一个装饰器，它把一个普通的方法转换成类方法：
    # 第一个参数是 cls（类本身），而不是 self（实例）可以访问和修改类变量，
    # 但不能访问实例变量可以通过类名直接调用，不需要创建实例：
    @classmethod
    def from_pretrained(cls, model_type):
        ...
        config_args['vocab_size'] = 50257 # for all gpt-2 series
        config_args['block_size'] = 1024  # for all gpt-2 series
        ...
        sd = model.state_dict()
        sd_keys = sd.keys()
        sd_keys = [k for k in sd_keys if not k.endswith('.attn.bias')] # omit the causal mask
...
class DataLoaderLite:
    ...
    def __init__(self, B, T, process_rank, num_processes, split):
        ...
        self.current_position = self.B * self.T * self.process_rank # diff start point
    ...
    def next_batch(self):
        ...
        # if loading the next batch would be out of bounds, advance to next shards
...
if ddp:
    ...
    ddp_rank = int(os.environ['RANK']) # global rank, multiple note
    ddp_local_rank = int(os.environ['LOCAL_RANK']) # local rank, withina single node
...
if ddp:
    model = DDP(model, device_ids=[ddp_local_rank]) # have to wrap the model into the DDP container
